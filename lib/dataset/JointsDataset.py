...
class JointsDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        coord_target, patch_target, target_weight = self.generate_target(joints, joints_vis)
        coord_target = torch.from_numpy(coord_target)
        patch_target = torch.from_numpy(patch_target)
        ...
        return input, coord_target, patch_target, target_weight, meta
    ...
    def generate_target(self, joints, joints_vis):
        ...
        for joint_id in range(self.num_joints):
            ...
            if v > 0.5:
                patch_target[joint_id][:] = rearrange(g, '(nh ph) (nw pw)-> (nh nw) pw ph', \
                nh = nh, ph = self.patch_size[1], 
                nw = nw, pw = self.patch_size[0]).sum(axis = (1, 2))
